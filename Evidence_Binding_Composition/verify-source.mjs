...
const declared = manifest.files.map((file) => file.path).filter((name) => name.endsWith(".thy") && !name.includes("/")).sort();
if (JSON.stringify(actual) !== JSON.stringify(declared)) throw Error("Theory inventory mismatch");
const sessionRoot = read("ROOT");
if (!sessionRoot.includes("session Evidence_Binding_Composition = Evidence_Atomic_Binding +")) {
...
for (const file of manifest.files) {
  ...
  if (file.path.endsWith(".thy") && !file.path.includes("/") && !sessionRoot.split(/\r?\n/).some((line) => line.trim() === file.path.slice(0, -4))) {
    throw Error(`Theory absent from ROOT: ${file.path}`);
  }
}
const parent = createHash("sha256").update(readFileSync(resolve(root, "../Evidence_Atomic_Binding/source-manifest.json"))).digest("hex");
...
const obligations = JSON.parse(read("../Evidence_Atomic_Binding/product-obligations.json"));
